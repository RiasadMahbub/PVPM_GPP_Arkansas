...
#### Same thing with constraining based on GPP 
LUEmaxGPPvpm<- list() ## LUEmax estimation GPP-APAR curve (list)
LUEmaxNEEvpm<-list() ## LUEmax estimation NEE-APAR curve (list)
LUEmaxGPPmaxvpm<- list() ## LUEmax estimation GPPmax-APAR curve (list)
LUEmaxNEEmaxvpm<-list()## LUEmax estimation NEEmax-APAR curve (list)
lightresponsegsvpm<-list() ## LUEmax estimation NEE curve from bigleaf package
modellistGPPvpm<-list() ## store the model result of the non linear regression of GPP curve
modellistNEEvpm<-list()
modellistGPPmaxvpm<-list() ## store the model result of the non linear regression of GPP curve
modellistNEEmaxvpm<-list()
plotrbase<- list()
...
directory_path <- "/Users/riasadbinmahbub/CodeRepository/R/GapfillingOtherRiceSites/GapfillingOtherRiceSites/Data/Satellite-SiteCalibrationCSV"
...
for (var_name in names(sheet_cols)) {  
  ...
  LUEmaxGPPvpm[[var_name]]<-rep(0,points[[var_name]]) ## this list will store the luemax values
  LUEmaxNEEvpm[[var_name]]<-rep(0,points[[var_name]])
  LUEmaxGPPmaxvpm[[var_name]]<-rep(0,points[[var_name]]) ## this list will store the luemax values
  LUEmaxNEEmaxvpm[[var_name]]<-rep(0,points[[var_name]])
  plotrbase[[var_name]]<-rep(0,points[[var_name]])
  ...
  for (i in 1:points[[var_name]]){
    ...
    modellistGPPvpm[[var_name]][[i]]<-data.frame()
    modellistNEEvpm[[var_name]][[i]]<-data.frame()
    modellistGPPmaxvpm[[var_name]][[i]]<-data.frame()
    modellistNEEmaxvpm[[var_name]][[i]]<-data.frame()
  ...
  for (i in (1:points[[var_name]])){
    ...
    ### constraining by vpm
    modellistGPPvpm[[var_name]][[i]]<-nlsLM((y) ~ (((a * (x) * g)/(a * (x)+ g))) * ts * ws * fpar * par *12.011, start=list(a= 0.03 ,g= 25),   ## what should the parameters value
                                         lower = c(a = 0, g = 0),  # Set lower bound for 'a'
                                         upper = c(a = 100, g =200) )
    
    modellistNEEvpm[[var_name]][[i]]<-nlsLM((-NEE ~ alpha * x / (1 - (x / 2000) + (alpha * x / GPP_ref)) - Reco),
                                         start=list(alpha=0.05,GPP_ref=30))
    ...
    LUEmaxGPPmax[[var_name]][[i]]<-modellistGPPmax[[var_name]][[i]][1,2] ### first row second column
    LUEmaxNEEmax[[var_name]][[i]]<-modellistNEEmax[[var_name]][[i]][1,2]
  ...
  for (i in 1:points[[var_name]]){
    dflist[[var_name]][[i]]$GPP_predicted_modellistGPP <- predict(modellistGPP[[var_name]][[i]])
    dflist[[var_name]][[i]]$NEE_predicted_modellistNEE <- (predict(modellistNEE[[var_name]][[i]]))
    #dflist[[var_name]][[i]]$GPP_predicted_modellistGPPmax <- (((LUEmaxGPPmax[[var_name]][[i]]  * (x) * g)/(LUEmaxGPPmax[[var_name]][[i]] * (x)+ g)))
    #dflist[[var_name]][[i]]$GPP_predicted_modellistNEEmax <- (LUEmaxNEEmax[[var_name]][[i]] * x / (1 - (x / 2000) + (LUEmaxNEEmax[[var_name]][[i]]  * x / GPP_ref)) - Reco)
    # ... (your existing code)
  }
...
##NEEbasedModels
plot_directory <- "/Users/riasadbinmahbub/RProgramming/GapfillingOtherRiceSites/GapfillingOtherRiceSites/Figure/LightResponseCurve/GPP_APAR_NLSM/NEEbased"
png(file.path(plot_directory, paste0(var_name, "_light_response_plot.png")), width = 800, height = 800)
for (var_name in names(sheet_cols)) {
  png(file.path(plot_directory, paste0(var_name, "_light_response_plot.png")), width = 800, height = 800)
  par(mfrow = c(5, 4))
  par(mar=c(2, 2, 2, 2))
  for (i in (1:points[[var_name]])){
    plot(dflist[[var_name]][[i]]$APAR, dflist[[var_name]][[i]]$NEE_modeled, xlab = "PAR", ylab = "NEE", col=dflist[[var_name]][[i]]$DOY)
    plot(sort(dflist[[var_name]][[i]]$APAR), dflist[[var_name]][[i]]$NEE_predicted_modellistNEE[order(dflist[[var_name]][[i]]$APAR)], col='red', lwd=2) 
  }
  dev.off() 
}
names(sheet_cols)
...
colnames(LUEmaxGPPdf) <- c("LUEmaxGPP", "SiteLUEmaxGPP")
colnames(LUEmaxNEEdf) <- c("LUEmaxNEE", "SiteLUEmaxNEE")
colnames(LUEmaxGPPmaxdf) <- c("LUEmaxGPPmax", "SiteLUEmaxGPPmax")
colnames(LUEmaxNEEmaxdf) <- c("LUEmaxNEEmax", "SiteLUEmaxNEEmax")
colnames(DOY_df) <- c("DOY", "SiteDOY_df")
# Rowbind all the list
LUEmax<- do.call("cbind", list(LUEmaxGPPdf, LUEmaxNEEdf, LUEmaxGPPmaxdf, LUEmaxNEEmaxdf, DOY_df))
LUEmax<-LUEmax %>% select("LUEmaxGPP", "LUEmaxNEE", "LUEmaxGPPmax", "LUEmaxNEEmax", "DOY", "SiteDOY_df")
LUEmax
plot(LUEmax$DOY, LUEmax$LUEmaxGPPmax)

### Save the LUEmax dataframe
# Assuming your dataframe is named LUEmax
write.csv(LUEmax, "Data/ExportedData/LUEmax.csv", row.names = FALSE)
...
calculate_gpp_vpm <- function(dataframe) {
  # Constants
  ...
  # Add GPP column to the dataframe
  dataframe$gpp <- gpp
