...
#### 500 meter 
ar_rice<-rasterlistCF[[4]]
ar_rice[ar_rice == 0] <- NA
crs <- "EPSG:3857" 
pr0 <- terra::project(stacked_raster_rast_mean, crs)
## now, snap that "out" to get clean alignment
ex <- align(ext(pr0), 500)
target <- rast(ex, res = c(500, 500), crs = crs)
stacked_raster_rast_mean_500<-terra::project(stacked_raster_rast_mean, target)
clay_raster_meanb30_500<-terra::project(clay_raster_meanb30, stacked_raster_rast_mean_500)
stacked_raster_rastEVI_500<-terra::project(stacked_raster_rast_meanEVI, stacked_raster_rast_mean_500)
...
stacked_raster_rastEVI_500df<-as.data.frame(stacked_raster_rastEVI_500, xy=TRUE)
stacked_raster_rastLSWI_500df<-as.data.frame(stacked_raster_rastLSWI_500, xy=TRUE)
stacked_raster_rastCF_500df<-as.data.frame(stacked_raster_rastCF_500, xy=TRUE)


GPPclay500df<-left_join(stacked_raster_rast_mean_500df, clay_raster_meanb30_500df, by = c("x" = "x", "y" = "y"))
GPPclay500df<-left_join(GPPclay500df, clay_raster_meanb30_500df, by = c("x" = "x", "y" = "y"))
GPPclay500df<-left_join(GPPclay500df, stacked_raster_rastEVI_500df, by = c("x" = "x", "y" = "y"))
GPPclay500df<-left_join(GPPclay500df, stacked_raster_rastLSWI_500df, by = c("x" = "x", "y" = "y"))
GPPclay500df<-left_join(GPPclay500df, stacked_raster_rastCF_500df, by = c("x" = "x", "y" = "y"))



###FIlter GPP based on 3 SD
single_shapefiledf<-as.data.frame(GPPclay500df)
...
mean_gppaggregate <- mean(single_shapefiledf$mean, na.rm = TRUE)
sd_gppaggregate <- sd(single_shapefiledf$mean, na.rm = TRUE)
...
filtered_df <- single_shapefiledf[single_shapefiledf$mean >= lower_bound &
                                    single_shapefiledf$mean <= upper_bound, ]
# Saving the filtered_df dataframe as a CSV file named "GPP500sdfilteredrgfclayevilswi"
# in the specified folder "C:/Users/rbmahbub/Documents/RProjects/VPMmodel/VPMmodel/Data/ExportedData"
...
# Remove rows where 'ar_rice' is NA
GPP500sdfilteredrgfclayevilswi <- GPP500sdfilteredrgfclayevilswi[complete.cases(GPP500sdfilteredrgfclayevilswi$ar_rice), ]

# Assuming single_shapefiledf is your dataframe
GPP500sdfilteredrgfclayevilswi <- GPP500sdfilteredrgfclayevilswi[GPP500sdfilteredrgfclayevilswi$ar_rice >= 1, ]

grgpp<-ggscatter(GPP500sdfilteredrgfclayevilswi, x = "ar_rice", y = "GPPggrg_8", size =0)+
  stat_cor(
...
grgpp
ggsave(("C:/Users/rbmahbub/Documents/RProjects/VPM_Spatial/Figure/RiceGrowingFrequency.png"), width = 14, height = 8)
...
slopeGPP
ggsave(("C:/Users/rbmahbub/Documents/RProjects/VPM_Spatial/Figure/EVISlope.png"), width = 14, height = 8)
...
# Save arranged plot
ggsave(
  filename = "C:/Users/rbmahbub/Documents/RProjects/VPM_Spatial/Figure/ricegrclayrgr.png",
  plot = arranged_plots,
...
# Assuming you have the ggplot2 and ggpmisc libraries installed
library(ggplot2)
library(ggpmisc)
...
EVIdataframe$geometry<-(single_shapefiledf$geometry)
GPPdataframe$geometry<-(single_shapefiledf$geometry)
GPPdataframe$rcgrwng <-single_shapefiledf$ar_rice
EVIdataframe$rcgrwng <-single_shapefiledf$ar_rice
